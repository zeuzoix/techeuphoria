...
the module cannot be built statically with the kernel.
...
The kernel Makefile is invoked as _KDIR_ is set and the rule for target _all_
will change to the _KDIR_ first and call the corresponding Makefile in that
...
way the module Makefile given below is invoked twice, the first time  from the
...
The kernel directory variable in the makefile must be configured as there
will be differences between different configurations of the kernel. The
module compiled against a specific version will only load in that kernel
version. If there is a mismatch then _insmod/modprobe_ will crib with an
output _"Invalid module format"_.
